<script lang="ts">
	...
	let { players: playerList, currentPlayerId, currentTurnPlayerId, hostId }: {
		players: typeof players.$inferSelect[]; currentPlayerId: string | null; currentTurnPlayerId: string | null; hostId: string;
	} = $props();
</script>
...
<div class="flex flex-col gap-2 w-full">
	...
	{#each playerList as player (player.id)}
		...
		<div class="flex items-center gap-2 sm:gap-3 rounded-xl px-3 py-2 transition-all duration-200 {isTurn ? 'bg-primary/15 border-2 border-primary/40' : 'bg-white/60 border border-border'}">
			...
			<span class="text-xs sm:text-sm font-semibold truncate max-w-[80px] sm:max-w-none {isTurn ? 'text-dark-gold' : 'text-text'}">
			...
			{#if isTurn}<div class="ml-auto h-2 w-2 rounded-full bg-primary sm:hidden"></div>{/if}
		</div>
	{/each}
</div>
